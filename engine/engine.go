...
func (ef *EngineFacilitator) takeForks() error {
	...
	if err != nil {
		ef.LogMessage("3", "takeForks, failure of getStateLeft", nil)
		return err
	}
	...
	if err != nil {
		ef.LogMessage("3", "takeForks, failure of getStateRight", nil)
		return err
	}
...
func (ef *EngineFacilitator) DpEngine(wg *sync.WaitGroup) {
	...
	for {
			...
			if ef.DpStates.State == handler.Thinking {
				...
				if i%globalData.SkipBetweenStateOutput == 0 {
					if globalData.DebugToStdout == "true" {
						fmt.Printf("\nfor in DpEngine\n")
						printState(ef.DpStates.State, ef.OwnAddress, ef.DpNumber)
						if ef.DpStates.State == handler.Quiesce3 {
		...
		err := ef.takeForks()
		if err != nil {
			ef.LogMessage("3", "DpEngine, takeForks call failed", err)
		}
		...
		err = ef.putForks()
		if err != nil {
			ef.LogMessage("3", "DpEngine, call to putForks failed", err)
		}
