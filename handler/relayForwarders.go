...
func (f *Facilitator) RelayDpResourceInformation(requestToProcess messageServerStack.ClientMessage) error {
	...
	if err != nil {
		f.LogMessage("C", "RelayDpResourceInformation, Marshal failure", err)
	...
	if err != nil {
		f.LogMessage("C", "RelayDpResourceInformation, build POST request failure", err)
	...
	if err != nil {
		f.LogMessage("C", "RelayDpResourceInformation, failed to send  POST request", err)
...
func (f *Facilitator) RelayDpAttributesRequest(currentAttributes DpAttributesRelay) error {
	...
	request, err := http.NewRequest("POST", requestUrl, bytes.NewBuffer(jsonData))
	if err != nil {
		f.LogMessage("C", "RelayDpAttributesRequest, build request failure", err)
	...
	if err != nil {
		if os.IsTimeout(err) {
			f.LogMessage("C", "RelayDpAttributesRequest, dpAttributesRelay request timed out after 30 seconds", err)
		} else {
			f.LogMessage("C", "RelayDpAttributesRequest, send request failure", err)
		}

		newAttributesPtr := &currentAttributes
		newAttributesPtr, err := f.AddEndOfLineData(f.LeftAddress, *newAttributesPtr)
		if err != nil {
			f.LogMessage("C", "RelayDpAttributesRequest, failed to complete AddEndOfLineData call", err)
		...
		f.LogMessage("I", msg, err)
		err = f.RelayDpAttributesReturnRequest(*newAttributesPtr)
		if err != nil {
			f.LogMessage("C", "RelayDpAttributesRequest, RelayDpAttributesReturnRequest failed", err)
...
func (f *Facilitator) RelayDpAttributesReturnRequest(currentAttributes DpAttributesRelay) error {
	...
	jsonData, err := json.Marshal(currentAttributes)
	if err != nil {
		f.LogMessage("C", "RelayDpAttributesReturnRequest, failed to marshal currentAttributes", err)
	...
	request, err := http.NewRequest("POST", requestUrl, bytes.NewBuffer(jsonData))
	if err != nil {
		f.LogMessage("C", "RelayDpAttributesReturnRequest, build request failure", err)
	...
	if err != nil {
		if os.IsTimeout(err) {
			f.LogMessage("W", "RelayDpAttributesReturnRequest, dpAttributesReturn request timed out after 30 seconds", err)
		...
		f.LogMessage("C", "RelayDpAttributesReturnRequest, send request failure", err)
		functionErr := NewForwardError(f.LeftAddress, f.OwnAddress, "RelayDpAttributesReturnRequest", "failed to send POST request", err.Error())
		return functionErr
	}
...
func (f *Facilitator) relayDpMessagesRequest(dpMessagesRelay DpMessagesRelay) error {
	...
	if err != nil {
		f.LogMessage("C", "relayDpMessagesRequest, failed to marshal dpMessagesRelay", err)
	...
	request, err := http.NewRequest("POST", requestUrl, bytes.NewBuffer(jsonData))
	if err != nil {
		f.LogMessage("C", "relayDpMessagesRequest, build request failure", err)
		functionErr := NewForwardError(f.LeftAddress, f.OwnAddress, "relayDpMessagesRequest", "failed to build POST request", err.Error())
		return functionErr
	}
	...
	response, err := client.Do(request)
	if err != nil {
		f.LogMessage("C", "relayDpMessagesRequest, send request failure", err)
...
func (f *Facilitator) addNewDpToLeftOfTargetDp(informationToAddNewDp *InformationToAddNewDp) error {
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, Marshal failure", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, failure to create setLeftAndRightAddressOfNewDpRequest", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp failure to send setLeftAndRightAddressOfNewDpRequest", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, failed to marshal data", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, failed to create requestChangeOfAddress", err)
		functionErr := NewForwardError(f.LeftAddress, f.OwnAddress, "addNewDpToLeftOfTargetDp", "failed to marshal requestDataChangeOfAddress", err.Error())
		return functionErr
	}
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, failure to send requestChangeOfAddress", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, build requestUrlToDeQuiesce1LeftOfTarget failure", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp of target, send request failure", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, build requestUrlToDeQuiesce3NewDP failure", err)
	...
	if err != nil {
		f.LogMessage("C", "addNewDpToLeftOfTargetDp, send requestToDeQuiesce3NewDP failure", err)
...
func (f *Facilitator) relayRequestToAddNewDpToLeftOfTargetDp(informationToAddNewDp *InformationToAddNewDp) error {
	...
	case "false":
		if informationToAddNewDp.OriginatorDpNumber == f.DpNumber && informationToAddNewDp.FromRequestHander == "false" {
			...
			if err != nil {
				f.LogMessage("C", "relayRequestToAddNewDpToLeftOfTargetDp, Marshal failure", err)
			...
			if err != nil {
				f.LogMessage("C", "relayRequestToAddNewDpToLeftOfTargetDp failure to create request", err)
			...
			if err != nil {
				f.LogMessage("C", "relayRequestToAddNewDpToLeftOfTargetDp failure to send request", err)
				functionErr := NewForwardError(f.LeftAddress, f.OwnAddress, "relayRequestToAddNewDpToLeftOfTargetDp", "failed to build POST request for AddNewDpToLeftOfTarget", err.Error())
				return functionErr
			}
	...
	case "true":
		if informationToAddNewDp.OriginatorDpNumber == f.DpNumber {
			...
			jsonDataInformationToAddNewDp, err := json.Marshal(informationToAddNewDp)
			if err != nil {
				f.LogMessage("C", "relayRequestToAddNewDpToLeftOfTargetDp marshal request data failure", err)
				functionErr := NewForwardError(f.LeftAddress, f.OwnAddress, "relayRequestToAddNewDpToLeftOfTargetDp", "failed to marshal informationToAddNewDp", err.Error())
				return functionErr
			}
			...
			handlerAddNewDpToLeftOfTargetDpRequest, err := http.NewRequest("POST", handlerAddNewDpToLeftOfTargetDpUrl, bytes.NewBuffer(jsonDataInformationToAddNewDp))
			if err != nil {
				f.LogMessage("C", "relayRequestToAddNewDpToLeftOfTargetDp failure to create request, %v\n", err)
	...
	default:
		// unknown Done value
		f.LogMessage("C", "relayRequestToAddNewDpToLeftOfTargetDp unknown informationToAddNewDp.Done value", nil)
...
func (f *Facilitator) removeDp() error {
	...
	if err != nil {
		f.LogMessage("C", "removeDp marshal failure", err)
	...
	if err != nil {
		f.LogMessage("C", "removeDp failure to create request", err)
	...
	if err != nil {
		...
		f.LogMessage("C", "removeDp failure to send request", err)
...
func (f *Facilitator) relayRequestToRemoveDp(informationToRemoveDp *InformationToRemoveDp) error {
	...
	case "false":
		if informationToRemoveDp.OriginatorDpNumber == f.DpNumber && informationToRemoveDp.FromRequestHander == "false" {
			...
			if err != nil {
				f.LogMessage("C", "relayRequestToRemoveDp, Marshal failure", err)
			...
			if err != nil {
				f.LogMessage("C", "relayRequestToRemoveDp failure to create POST request", err)
			...
			if err != nil {
				f.LogMessage("C", "relayRequestToRemoveDp failure to send request", err)
	...
	default:
		// unknown Done value
		f.LogMessage("C", "relayRequestToRemoveDp unknown informationToAddNewDp.Done value", nil)
