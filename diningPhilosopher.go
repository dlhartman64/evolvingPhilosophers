...
func main() {
	...
	detachFromTerminalInitialRingPtr := initialRingCmd.String("detachFromTerminal", "false", "Optional debug output to stdout")
	...
	detachFromTerminalAddToRingPtr := addToRingCmd.String("detachFromTerminal", "false", "Optional debug output to stdout")
	...
	if command == "initialRing" || command == "directAddToRing" {
		if daemon {
			initialRingCmd.Parse(os.Args[3:])
		} else {
		...
		if *detachFromTerminalInitialRingPtr == "true" && !daemon {
		...
		if globalData.DebugToStdout == "true" {
			...
			fmt.Println("	detachFromTerminal:", *detachFromTerminalInitialRingPtr)
		}
	}

	if command == "addToRing" {
		if daemon {
			addToRingCmd.Parse(os.Args[3:])
		} else {
		...
		if *detachFromTerminalAddToRingPtr == "true" && !daemon {
			detachFromTerminal = true
		}
		...
		if globalData.DebugToStdout == "true" {
			...
			fmt.Println("	iterationsToSkipBetweenStateDebugOutput:", *iterationsToSkipBetweenStateDebugOutputInitialRingPtr)
			fmt.Println("	debugStdout:", *debugStdoutInitialRingPtr)
			fmt.Println("	detachFromTerminal:", *detachFromTerminalAddToRingPtr)
		}
	}

	if detachFromTerminal {
		if globalData.DebugToStdout == "true" {
			fmt.Printf("Detaching from terminal\n")
		}
		...
		case 2:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1])
		case 3:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1], os.Args[2])
		case 4:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1], os.Args[2], os.Args[3])
		case 5:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1], os.Args[2], os.Args[3], os.Args[4])
		case 6:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1], os.Args[2], os.Args[3], os.Args[4], os.Args[5])
		case 7:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1], os.Args[2], os.Args[3], os.Args[4], os.Args[5], os.Args[6])
		case 8:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1], os.Args[2], os.Args[3], os.Args[4], os.Args[5], os.Args[6], os.Args[7])
		case 9:
			cmd = exec.Command(os.Args[0], "daemon", os.Args[1], os.Args[2], os.Args[3], os.Args[4], os.Args[5], os.Args[6], os.Args[7], os.Args[8])
		default:
			if globalData.DebugToStdout == "true" {
		...
		nullFile, nullFileErr := os.OpenFile(os.DevNull, os.O_RDWR, 0)
		...
		if err := cmd.Start(); err != nil {
			if globalData.DebugToStdout == "true" {
				fmt.Printf("Error detaching from terminal, ending process: %v\n", err)
			}
			os.Exit(1)
		}
		os.Exit(0)
